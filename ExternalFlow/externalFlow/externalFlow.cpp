...
void setParam(std::string xmlInputFileName)
{
    // User input data (all in physical units).

    XMLreader document(xmlInputFileName);

    document["input"]["lx"].read(param.lx);
    document["input"]["ly"].read(param.ly);
    document["input"]["lz"].read(param.lz);
    document["input"]["xSide"].read(param.xSide);
    document["input"]["ySide"].read(param.ySide);
    document["input"]["resolution"].read(param.resolution);
    document["input"]["dt"].read(param.dt);
    document["input"]["maxIter"].read(param.maxIter);
    document["input"]["statIter"].read(param.statIter);
    document["input"]["outIter"].read(param.outIter);
    document["input"]["startIter"].read(param.startIter);
    document["input"]["inletVelocity"].read(param.inletVelocity);
    document["input"]["Re"].read(param.Re);
    param.nu = param.inletVelocity * param.ySide / param.Re;
    document["input"]["mountPoint0"].read(param.mountPoint[0]);
    document["input"]["mountPoint1"].read(param.mountPoint[1]);
    document["input"]["mountPoint2"].read(param.mountPoint[2]);
    document["input"]["ibIter"].read(param.ibIter);
    document["input"]["periodicBoundaries"].read(param.periodicBoundaries);
    param.ampl = 0.25 * pi;
    document["input"]["freq"].read(param.freq);
    document["input"]["model"].read(param.model);
...
int main(int argc, char* argv[])
{
    ...
    // add new
    TriangleSet<T>* rectangleTriangleSet = new TriangleSet<T>(param.model, 0.001);
    //////
